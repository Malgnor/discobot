...
class Painter(Plugin):
    ...
    def on_painterdraw_command(self, event):
        ...
        try:
            img = Image.new('RGBA', (ctx.get('width', 100), ctx.get('height', 100)), getrgb(ctx.get('bgcolor', 'white')))
    ...
    def on_paintertextadd_command(self, event, x, y, color, font, size, text):
        try:
            font = [f[0]+f[1] for f in self.fonts if f[0] == font]
            if len(font) == 0:
            ...
            event.msg.reply('Tamanho do texto em pixels: `{}`'.format(fnt.getsize(text)))
            del fnt
        except Exception as e:
            event.msg.reply('[ERRO]{}'.format(e))        
